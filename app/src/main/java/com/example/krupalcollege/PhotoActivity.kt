...
class PhotoActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        super.onResume()
        database = FirebaseDatabase.getInstance().getReference("Photo")  // ✅ Reinitialize reference
    ...
    private fun fetchCakesFromFirebase() {
        database.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (cakeList.isEmpty()) {
                    Toast.makeText(this@PhotoActivity, "No cakes found!", Toast.LENGTH_SHORT).show()
                }
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@PhotoActivity, "Failed to load data", Toast.LENGTH_SHORT).show()
